...
function SymbolicUncertainties.parse_measurement_model(m::CausalGraphs.MeasurementModel)
    ...
    push!(lines, "$out_name = " * join([string(CausalGraphs.name(n)) * "_meas" for n in CausalGraphs.inputs(m)], " + ") * " # <--- edit this")
...
"""
function SymbolicUncertainties.evaluate_measurement_model(m::CausalGraphs.MeasurementModel)
